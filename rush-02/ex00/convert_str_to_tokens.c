...
/*   Updated: 2026/02/22 21:20:06 by smiuccio         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < get_len(str))
	{
		...
		while (nbrs[i][j])
		{
			if (ft_strlen(nbrs[i]) == 3)
			{
				...
				else if (j == 1 && (nbrs[i][j] % 100) > 19)
				{
					ft_putstr(get(dict, &nbrs[i][j]));
					ft_putstr(" ");
					ft_putstr(get(dict, &nbrs[i][j + 1]));
					ft_putstr(" ");
				}
				else if (j == 1 && (nbrs[i][j] % 100) <= 19)
				{
					value = ft_strcat(&nbrs[i][j + 1], &nbrs[i][j]);
					ft_putstr(get(dict, value));
					ft_putstr(" ");
				}
				else if (j == 2)
				{
			...
			else if (ft_strlen(nbrs[i]) == 2)
			{
				if (j == 1 && (nbrs[i][j] % 100) > 19)
				{
					ft_putstr(get(dict, &nbrs[i][j]));
					ft_putstr(" ");
					ft_putstr(get(dict, &nbrs[i][j + 1]));
