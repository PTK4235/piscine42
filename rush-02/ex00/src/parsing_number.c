...
/*   Updated: 2026/02/22 15:57:40 by mpourbai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**parsing_number(char *nbr, char **nbrs)
{
	...
	if (!nbrs)
		printf("error malloc 1\n"); // TODO remplacer printf avec write
	i = 0;
	while (i < (ceil))
	{
		if (len % 3 != 0 && i == 0)
	...
	i = 0;
	c = 0;
	while (i < (ceil)) // iterer sur la str du nb total
	{
		j = 0;
		if (countfirst > 0 && countfirst < 3) // cas specifique ou il y a des restes
		{
